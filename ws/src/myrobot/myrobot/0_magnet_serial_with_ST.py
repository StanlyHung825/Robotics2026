## real arm with electromagnet support (Axis2 and Axis3 relationship is cared)
import struct
import threading
...
class StmJointAnglePublisher:
    ...
    def pub(self, cmd_jp: list[float]) -> None:
        ...
        """
        if len(cmd_jp) != 5:
            raise ValueError("Command must have 5 elements")

        # First 4: joint angles (in rad, scaled by 1000)
        # 5th: end effector state (0 or 1)
        int_cmds = [int(x * 1000) for x in cmd_jp[:4]] + [int(cmd_jp[4])]
        packed_data = struct.pack("<5i", *int_cmds)
...
class STM32MagnetInterface(Node):
    def __init__(self):
        super().__init__("stm32_magnet_interface")
        ...
        self.get_logger().info("STM32MagnetInterface node has been started")

    def callback(self, msg: Float64MultiArray):
        cmd = list(msg.data)
        if len(cmd) < 5:
            self.get_logger().error(f"Received commands array with fewer than 5 elements: {cmd}")
            return
        cmd[2] += cmd[1] * 0.7  # belt transmission of real arm
